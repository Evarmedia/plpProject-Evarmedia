...
CREATE TABLE IF NOT EXISTS Communities (
    ...
    ON DELETE SET NULL -- Prevents deletion of the community manager from affecting the community
);
...
CREATE TABLE IF NOT EXISTS CommunityManagers (
    ...
    ON DELETE CASCADE, -- Deletes the user when the community manager is deleted
...
CREATE TABLE IF NOT EXISTS Donations (
    ...
    FOREIGN KEY (user_id) REFERENCES Users(user_id)
    ON DELETE CASCADE, -- Deletes the donations when the user is deleted
    FOREIGN KEY (community_id) REFERENCES Communities(community_id)
);
